...
class BombproofAITradingSystem extends EventEmitter {
  ...
  // TRADE EXIT HANDLER (basic, emits tradeCompleted)
  async handleTradeExit(tradeId, positionState) {
    try {
      const trade = this.executedTrades.get(tradeId);
      if (!trade) return;

      const exitPrice = positionState.currentPrice || trade.price;
      const priceDiff = trade.direction === 'long' ? exitPrice - trade.price : trade.price - exitPrice;
      const finalPnL = priceDiff * trade.quantity;
      ...
      const result = {
        ...
        exitTime: new Date(),
        predictionConfidence: trade.confidence || 0,
        trade
      };
      ...
      // Update risk state
      this.riskState.dailyPnL += finalPnL;

      // Emit
      this.emit('tradeCompleted', result);

    } catch (err) {
      logger.error('Error handling trade exit:', err);
    }
  }
}
