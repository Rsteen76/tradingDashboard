...
class AIPerformanceMonitor extends EventEmitter {
  ...
  setupEventListeners() {
    ...
    // ML events - handle gracefully if not an EventEmitter
    if (this.mlEngine && typeof this.mlEngine.on === 'function') {
      ...
      logger.debug('ML Engine is not an EventEmitter, skipping ML event listeners');
    }
  ...
  checkAlertConditions() {
    // Check for alert conditions during monitoring loop
    try {
      ...
      // Check win rate
      if (this.metrics.trading.winRate < this.alertThresholds.minWinRate && this.metrics.trading.dailyPnL !== 0) {
        this.triggerAlert('low_win_rate', {
      ...
      // Check system health
      if (this.metrics.health.score < this.alertThresholds.minSystemHealth) {
        this.triggerAlert('system_health', {
          score: this.metrics.health.score,
          threshold: this.alertThresholds.minSystemHealth
        });
      }
  ...
  async loadHistoricalMetrics() {
    try {
      ...
      const filePath = path.join(__dirname, '../../../data/performance-metrics.json');
      ...
      logger.info('Historical performance metrics loaded');
    } catch (error) {
      logger.debug('No historical performance metrics found, starting fresh');
    }
  }
